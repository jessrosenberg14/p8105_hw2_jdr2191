...
The Mr. Trash Wheel dataset contains `r count(mrtrashwheel_df)` observations. In 2019, the median number of sports balls in a dumpster was  `r mrtrashwheel_df %>% filter(year == 2019) %>% summarise(median(sports_balls))` balls.

The precipitation dataset contains `r count(precipitation_df)` observations. In 2018, the total precipitation was `r precipitation_df %>% filter(year == 2018) %>% summarise(sum(total))` inches.
